...
1. SSID/password
2. ESP32 is within Wi-Fi range
3. The network is available to the ESP32
4. Serial Monitor output at 115200 baud
...
### MQ135 value looks wrong

Check the voltage divider and remember that MQ135 readings depend strongly on sensor warm-up, calibration, supply voltage and environment.

### Rain status is inverted

Reverse the `LOW`/`HIGH` logic described in the wiring section.

## 📌 Original project

This repository is the ESP32/local-HTTP migration of the original Arduino Uno R4 WiFi weather monitoring project.
...
MQTT/cloud communication
            ↓
ESP32-hosted local HTTP server
```
